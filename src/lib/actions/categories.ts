...
/** What this category's goods are taxed at.
 *
 * `null` clears it, and clearing is not the same as setting zero: a cleared
 * category is taxed at the shop's rate and follows it when it changes, while
 * a category set to 0 stays untaxed whatever the shop does later. The two
 * are different promises and the UI offers both.
 *
 * The PERCENTAGE is what a person types; the column holds the fraction,
 * matching settings.tax_rate and lib/money.ts. Storing what was typed would
 * make 10 mean 1000% to every reader of the column.
 */
export async function setCategoryTaxRate(id: string, percent: number | null) {
  await requireAdmin();
  const sb = supabaseAdmin();
  ...
  if (percent != null) {
    ...
    // Refused rather than clamped: 250 in the box is somebody typing into
    // the wrong field, and quietly storing 100% is not a kindness.
    if (!Number.isFinite(n) || n < 0 || n > 100) {
      throw new Error("A tax rate must be between 0 and 100 percent");
    }
    rate = Math.round(n * 1e4) / 1e6;   // 2.5 -> 0.025
  }

  const { error } = await sb.from("categories").update({ tax_rate: rate }).eq("id", id);
  // A database that has not run supabase/public-settings-grant.sql has no
  // such column. Say so plainly instead of letting a PostgREST error code
  // reach the owner as a toast.
  if (error) {
    throw new Error(
      /column .*tax_rate/i.test(error.message)
        ? "Run supabase/public-settings-grant.sql first — this database has no per-category tax column yet."
        : error.message);
  }
  revalidatePath("/", "layout");
  updateTag(CACHE_TAGS.categories);
