...
import { encodeActor, decodeActor, OWNER, type AdminActor } from "@/lib/session";

const NOW = Date.parse("2026-09-02T10:00:00Z");
const TEN_MIN = 10 * 60 * 1000;
const staff: AdminActor = { kind: "staff", id: "u-1", label: "Ana" };

describe("session payload", () => {
  it("round-trips the owner", () => {
    expect(decodeActor(encodeActor(OWNER, NOW), NOW, TEN_MIN)).toEqual(OWNER);
  });
  ...
  it("survives a name that would break a colon-delimited token", () => {
    const awkward: AdminActor = { kind: "staff", id: "u-2", label: "Ana: Sales . v2" };
  ...
  it("still accepts a session issued before named accounts existed", () => {
    ...
    expect(decodeActor(`admin:${NOW}`, NOW, TEN_MIN)).toEqual(OWNER);
