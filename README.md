...
| **Order notifications** | The buyer is messaged when their order is placed and at each status change, with a signed one-tap link to their order — no phone number to retype. An outbox (queue first, send second) means a message is never lost to a failed API call. Works with zero setup via one-tap WhatsApp links in `/admin/notifications`, and sends itself once the WhatsApp Cloud API is configured. See [DEPLOY.md](./DEPLOY.md#order-notifications). |
