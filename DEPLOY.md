...
**One file, once.** Open `supabase/run-all.sql`, paste the whole thing into
**SQL Editor → New query**, press **Run**.

That file is every migration in this repository concatenated in the order
they have to be run — generated by `node scripts/build-run-all.mjs` from
`SCHEMA_ORDER` in `src/lib/schemaHealth.ts`, with a test that keeps it in
step with the folder and honours every "Run AFTER" the files state about
each other.

Every file in it is safe to re-run, so this is also how you catch an
existing database up after pulling new features — not only how you create
one. Run it again whenever the **Settings → Schema health** panel says
something is outstanding.

> **This section used to list six files by name and there are twenty-eight.**
> The nineteen it never mentioned included the stock ledger, the payment
> tables, staff logins, and the two files whose entire purpose is closing
> security holes — so following it literally produced a shop missing all of
> them. That is what `run-all.sql` exists to prevent. If you would rather
> run them one at a time, the order is the one inside that file.
